...
export default function Page() {
  const { leaveState, setLeaveState, teachersState, setTeachersState } =
    useGlobalContext();
  ...
  const getData = async () => {
    ...
    try {
      ...
      !tData?.id && navigate("/");
      setTname(tData.tname);
      setDesig(tData.desig);
      setSchool(tData.school);
      setDoj(tData.doj);
      ...
      if (leaveState.length === 0) {
        const querySnapshot = await getDocs(
          query(collection(firestore, "leaves"))
        );
      ...
      setLoader(false);
      const prevLeaves = data.filter((item) => item.teachersId === tData.id);
